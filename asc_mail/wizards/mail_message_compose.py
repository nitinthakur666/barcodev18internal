...
class MailComposeMessage(models.TransientModel):
    ...
    def _prepare_mail_values(self, res_ids):
        """Add CC and BCC before mail creation"""
        mail_values = super()._prepare_mail_values(res_ids)
        # mail_values is a dict: {res_id: {mail fields}}
        for res_id, values in mail_values.items():
            cc_partner_ids = self.cc_partner_ids.ids if self.cc_partner_ids else []
            bcc_partner_ids = self.bcc_partner_ids.ids if self.bcc_partner_ids else []
            values.update({
                'cc_partner_ids': cc_partner_ids,
                'bcc_partner_ids': bcc_partner_ids,
            })
        return mail_values