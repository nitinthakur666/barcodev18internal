...
class SaleOrder(models.Model):
    ...
    bci_location = fields.Many2one('stock.location', 'BCI Location',
                                   default=lambda self: self.env['stock.location'].search(
                                       [('bci_code', 'ilike', 'BCILGN')], limit=1).id)
    bci_stage = fields.Selection(
        [('draft', 'Draft'), ('pricing_request', 'Pricing Request'), ('pricing_updated', 'Pricing Updated'),
         ('pending_approval', 'Pending Approval'), ('approved', 'Approved'), ('rejected', 'Rejected')], string='Stage',
        default='draft', readonly=False, compute='_compute_bci_stage', store=True, tracking=True, copy=False)
    ...
    bci_category_summary = fields.One2many('barcode_india.category_summary', 'bci_sale_id', string='Category Summary',
                                           copy=False)
    bci_payment_terms = fields.One2many('barcode_india.payment_term', 'order_id', string='Payment Terms', copy=True,
                                        tracking=True)
    bci_readonly_fields = fields.Char('Readonly Fields')
    bci_user_role = fields.Selection([('guest', 'Guest'), ('pre_sales', 'Pre Sales'), ('sale_person', 'Sale Person'),
                                      ('category_head', 'Category Head'), ('management', 'Management')],
                                     compute='_compute_user_role', string='Roles')
    bci_validity_expired = fields.Boolean('Is Validity Expired?')
    validity_expired_message = fields.Char(string='Validity Expired Message',
                                           compute='_compute_validity_expired_message')
    bci_exchange_expired = fields.Boolean('Is exchange Expired?')
    exchange_expired_message = fields.Char(string='Exchange Expired Message',
                                           compute='_compute_exchange_expired_message')
    ...
    bci_pt_approval = fields.Selection([('pending', 'Pending'), ('approved', 'Approved')],
                                       string="Payment Term Approval Status", default='approved', copy=False)
    bci_margin_approval = fields.Selection([('pending', 'Pending'), ('approved', 'Approved')],
                                           string="Margin Approval Status?", default='pending', copy=False)
    bci_special_approval = fields.Selection([('pending', 'Pending'), ('approved', 'Approved')],
                                            string="Special Approval Status?", default='pending', copy=False)
    bci_printout_type = fields.Selection(
        [('section', 'Section'), ('sub_section', 'Sub Section'), ('details', 'Details')], string="Type",
        default='details')
    ...
    bci_bg_pg = fields.Selection([('bg', 'ABG'), ('pbg', 'PBG')],
                                 string='Any ABG/PBG for Payment. Will need additional Approvals?')
    bci_payment_period = fields.Selection(
        [('within_45_days', 'As per Payment Terms'), ('beyond_45_days', 'More than 30 Days')], default='within_45_days',
        string='Payment period. Anything beyond 30 days will need additional Approvals.')
    ...
    bci_presales_ids = fields.Many2many('res.users', related='opportunity_id.bci_presales_ids', string='Pre-Sales',
                                        readonly=True)
    ...
    po_attach_ids = fields.Many2many('ir.attachment', string='Purchase Order Attachments',
                                     help='Attach one or more files if needed.')
    special_instructions = fields.Text(string="Special Instructions")
    bci_is_estimation = fields.Boolean(related='bci_quote_type.estimation', string="Estimation")
    check_quote_type_message = fields.Char(string='Message :',
                                           default="Please set a value in 'Quote Type' before editing the order lines!")
    ...
    is_sopf_button_visible = fields.Boolean(string='Is SOPF Button Visible', compute='_compute_is_sopf_button_visible',
                                            store=True)
    ...
    partner_credit_limit = fields.Float(related='partner_id.bci_outstanding_balance_exceed',
                                        string="Customer Credit Limit", store=True)
    partner_outstanding_balance = fields.Float(related='partner_id.bci_outstanding_balance',
                                               string="Customer Outstanding Balance", store=True, )
    ...
    is_reference = fields.Boolean(string="Is Reference",
                                  help="If checked, the reference cost sheet will be used to populate the current cost sheet.")
    reference_costsheet_validity = fields.Date(string='Reference Costsheet Validity', tracking=True)
    reference_company_ids = fields.Many2many('res.partner', 'bci_reference_company_rel', string='Reference Companys',
                                             tracking=True)
    bci_lob = fields.Many2one('barcode_india.lob', string='LOB')
    bci_company_code = fields.Char(string="Company Code", compute="_compute_bci_company_code", store=True)

    @api.depends('partner_id', 'partner_id.parent_id')
    def _compute_bci_company_code(self):
        for record in self:
            if record.partner_id and record.partner_id.bci_code and record.partner_id.is_company:
                record.bci_company_code = record.partner_id.bci_code
            elif record.partner_id and record.partner_id.parent_id and record.partner_id.parent_id.bci_code and record.partner_id.parent_id.is_company:
                record.bci_company_code = record.partner_id.parent_id.bci_code
            else:
                record.bci_company_code = False
    ...
    def create(self, vals):
        ...
        if order.state != 'SOPF' and order.order_line and all(
                line.bci_approval_stage == 'approved' for line in order.order_line):
    ...
    def send_for_approval(self):
        for rec in self:
            ...
            if not rec.estimation:
                ...
                installation_product_id = int(
                    self.env['ir.config_parameter'].sudo().get_param('bci.installation_product'))
            ...
            if rec.bci_pt_approval and rec.bci_pt_approval != 'approved' and not rec.bypass_quote:
                ...
                margin_lines = self.env['barcode_india.pt_matrix'].sudo().search(
                    [('quotations_type_id', '=', rec.bci_quote_type.id), '|', ('value', '>=', price_subtotal), '|',
                     ('customer_type', '=', rec.partner_id.bci_customer_type), ('customer_type', '=', 'all'), '|',
                     ('customer_category', '=', rec.partner_id.bci_customer_category),
                     ('customer_category', '=', 'all')], order='sequence asc')
                if margin_lines:
                    for margin_line in margin_lines:
                        if margin_line.recommending_authority == 'sale_person' or margin_line.approving_authority == 'sale_person':
                            ...
                            sale_person_limit = self.env['barcode_india.sale_person_limit'].sudo().search(
                                [('quot_type', '=', quote_type.id), ('user', '=', sale_person.id),
                                 ('payment_deal_value', '>', rec.amount_total)])
                        ...
                        if margin_line.recommending_authority == 'user':
                            ...
                            vertical_head_id = extract_user_ids(
                                rec.partner_id and rec.partner_id.parent_id and rec.partner_id.parent_id.bci_vertical and rec.partner_id.parent_id.bci_vertical.vertical_head or (
                                            rec.partner_id.bci_vertical and rec.partner_id.bci_vertical.vertical_head))
                            ...
                            region_head_id = extract_user_ids(
                                rec.user_id.partner_id and rec.user_id.partner_id.parent_id and rec.user_id.partner_id.parent_id.bci_region_user and rec.user_id.partner_id.parent_id.bci_region_user.region_head or (
                ...
                approval_request.approver_ids = [(0, 0, {'user_id': user_id, 'required': True}) for user_id in
                                                 unique_user_ids]
            ...
            margin_category = self.env['approval.category'].sudo().search([('bci_is_margin_approval', '=', True)],
                                                                          limit=1)
            request_model = self.env['approval.request'].sudo()
            ...
            def create_approval_request(approver_ids):
                if approver_ids:
                    ...
                    approval_request.approver_ids = [(0, 0, {'user_id': user_id, 'required': True}) for user_id in
                                                     approver_ids]
                    approval_request.action_confirm()
                    all_approvers.update(approver_ids)
            ...
            def handle_approver_type(margin):
                ...
                if margin.approver_type == 'sale_person':
                    sale_person = rec.user_id
                    if sale_person:
                        sale_person_limit = self.env['barcode_india.sale_person_limit'].search(
                            [('quot_type', '=', margin.quotation_type_id.id), ('user', '=', sale_person.id)], limit=1)
            ...
            if not rec.estimation:
                if rec.bci_auto_approval <= round(rec.bci_deal_margin, 2):
                    ...
                    approver_ids = []
                    applicable_margin = min(quote_type.margin_ids,
                                            key=lambda m: m.deal_margin if m.deal_margin == 0 else float('inf'))
            ...
            if rec.bci_special_approval != 'approved' and not rec.bypass_quote:
                ...
                if special_approvers:
                    approval_requests_exist = True
                    special_category_id = self.env['approval.category'].sudo().search(
                        [('bci_is_special_approval', '=', True)])
                    request_model = self.env['approval.request'].sudo()
        ...
        for order in self:
            if not order.estimation:
                ...
                installation_product_id = {
                    'bci_installation_product': self.env['ir.config_parameter'].sudo().get_param(
                        'bci.installation_product'),
                }
                ...
                if order.bci_installation_type:
                    installation_amount = total_price_installation * (
                                order.bci_installation_type.percentage_of_Hardware / 100)
                else:
                ...
                if freight_applicable:
                    freight_product_template = self.env['product.template'].sudo().search(
                        [('id', '=', freight_product_id['bci_freight_product'])])
                ...
                if installation_applicable:
                    installation_product_template = self.env['product.template'].sudo().search(
                        [('id', '=', installation_product_id['bci_installation_product'])])
    ...
    def _onchange_quote_type(self):
        for order in self:
            ...
            if order.bci_quote_type:
                ...
                bullet_points = ['<li style="font-size: 20px;"><b>{}</b> : {}</li>'.format(html_escape(t.name),
                                                                                           html_escape(
                                                                                               t.bci_description)) for t
                                 in tnc]
                order.note = '<ol>{}</ol>'.format('<br>'.join(bullet_points))
            else:
                order.note = False

    @api.depends('order_line.bci_approval_stage', 'bci_pt_approval', 'bci_margin_approval', 'bci_quote_type',
                 'bci_special_approval')
    def _compute_bci_stage(self):
        for order in self:

            if order.state != 'SOPF' and order.order_line and all(line.bci_approval_stage == 'approved' for line in
                                                                  order.order_line) and order.bci_stage != 'pending_approval':
                order.bci_stage = 'pricing_updated'
    ...
    @api.depends('order_line.price_unit', 'order_line.bci_landed_cost', 'order_line.product_uom_qty',
                 'order_line.bci_discount')
    def _compute_deal_margin(self):
        for order in self:
            freight_product_id = int(self.env['ir.config_parameter'].sudo().get_param('bci.freight_product'))
            installation_product_id = int(self.env['ir.config_parameter'].sudo().get_param('bci.installation_product'))
            landed_price_total = sum(order.order_line.filtered(
                lambda line: line.product_template_id.id not in [freight_product_id, installation_product_id]).mapped(
                lambda line: (line.bci_purchase_cost * line.product_uom_qty))) or 0.00
            price_total = sum(order.order_line.filtered(
                lambda line: line.product_template_id.id not in [freight_product_id, installation_product_id]).mapped(
                lambda line: (line.price_unit * line.product_uom_qty))) or 0.01
            order.bci_deal_margin = (1 - (landed_price_total / price_total)) * 100 or 0

    def _action_confirm(self):
        for order in self:
            if order.bci_helpdesk:
                in_progress_stage = self.env['helpdesk.stage'].sudo().search(
                    [('bci_in_progress_stage', '=', True)]).filtered(
                    lambda x: order.bci_helpdesk.team_id.id in x.team_ids.ids)
            ...
            if not partner_id.bci_group_code and not partner_id.bci_tax_group and not partner_id.bci_terms_code:
                raise UserError("Please check  - Group Code / Tax Group / Terms Code mapped on the Customer (%s)" % (
                    partner_id.name))
            if not partner_id.bci_vertical or not partner_id.bci_sub_vertical or not partner_id.bci_sub_vertical_category:
                raise UserError(
                    f"Vertical / Sub Vertical / Sub Vertical Category is mandatory for customer {partner_id.name}")
    ...
    def action_view_cost_sheet(self):
        self.ensure_one()
        view_record = self.env['barcode_india.view_master'].sudo().search(
            [('bci_role', '=', self.bci_user_role), ('bci_stage', '=', self.bci_stage)], limit=1) or False
    ...
    def _get_default_payment_terms(self):
        ...
        if not self.is_new_version:
            ...
            if self.bci_quote_type and self.partner_id.bci_pricing_terms:
                partner_payment_terms = self.partner_id.bci_pricing_terms.filtered(
                    lambda x: x.bci_quote_type.id == self.bci_quote_type.id)
    ...
    def _set_validity_expired(self, limit=False):
        orders = self.search([('bci_validity_expired', '=', False), ('state', 'in', ['draft', 'sent']),
                              ('validity_date', '<=', fields.Date.today())], limit=limit)
        if orders:
            for order in orders:
    ...
    def _check_exchange_diff(self, limit=False):
        ...
        for currency in currencies:
            ...
            exchange_diff_minus_percentage = (
                        base_currency_rate - base_currency_rate * exchange_threshold / 100) if base_currency_rate != 0 else 0
            exchange_diff_plus_percentage = (
                        base_currency_rate + base_currency_rate * exchange_threshold / 100) if base_currency_rate != 0 else 0
            orders = self.search([('bci_exchange_expired', '=', False), ('state', 'in', ['draft', 'sent']),
                                  ('order_line.bci_purchase_currency', '=', currency.id), '|',
                                  ('order_line.bci_exchange_rate', '<', exchange_diff_minus_percentage),
                                  ('order_line.bci_exchange_rate', '>', exchange_diff_plus_percentage)], limit=limit)
            if orders:
                orders.write({'bci_exchange_expired': True})
                for order in orders:
                    message = _(
                        "Expiring validity for this order because the difference between the exchange rate at the time of order and the current exchange rate exceeds the set threshold .")
                    order.message_post(body=message)

    def _check_cost_diff(self, limit=False):
        cost_threshold = float(self.env['ir.config_parameter'].sudo().get_param('bci.cost_threshold'))
        orders = self.env['sale.order'].search([('bci_cost_expired', '=', False), ('state', 'in', ['draft', 'sent']),
                                                ('order_line.product_id.bci_purchase_cost_change', '=', True),
                                                ('order_line.product_id.bci_purchase_currency', '!=',
                                                 self.env.company.currency_id.id)], limit=limit)
        if orders:
            ...
            if order_lines:
                ...
                if all(abs(cost_diff_minus_percentage) < cost_threshold or abs(
                        cost_diff_plus_percentage) > cost_threshold):
                    # raise Exception(orders)
                    orders.write({'bci_cost_expired': True})
                    for order in orders:
                        message = _(
                            "Expiring validity for these orders because the difference between the purchase cost at the time of order and the current purchase cost exceeds the set threshold .")
                        order.message_post(body=message)
        ...
        for order in self:
            ...
            if order.company_id.tax_calculation_rounding_method == 'round_globally':
                ...
                for line in order_lines:
                    ...
                    amount_untaxed += tax_results.get('total_excluded', 0.0)
                    amount_tax += tax_results.get('total_included', 0.0) - tax_results.get('total_excluded', 0.0)
